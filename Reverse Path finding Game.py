...
def draw_instructions(win):
    ...
    instructions = [
        ...
        "5. After the path is drawn Press Y and wait 2 seconds for th borad to reset"
    ]
...
def reset_game(reset_start_goal=False):
    ...
    grid = [[0 for _ in range(COLS)] for _ in range(ROWS)]
    open_set, came_from, searched, path = [], {}, set(), []
    finished = False
    elapsed_time = 0
    win_message = ""
    if reset_start_goal:
    ...
    while running:
        ...
        if finished:
            if next_level:
                ...
                start_time = None
                elapsed_time = 0
                win_message = "You won! Would you like to go to the next level?"
                draw_text(win, win_message, (WIDTH // 2 - 100, HEIGHT // 2), font_size=40, color=RED)
                draw_text(win, "Press Y for Yes or N for No", (WIDTH // 2 - 150, HEIGHT // 2), font_size=30)
        ...
        draw_text(win, f"Time Taken: {int(elapsed_time)}s", (10, 70), font_size=25)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_SPACE and start and goal and not finished:
                    ...
                    if elapsed_time <= level_time_limit and finished:
                        draw_text(win, "You won! Would you like to go to the next level?", (WIDTH // 2 - 250, HEIGHT // 2 - 50), font_size=30)
                        
                        if event.key == pygame.K_y and finished:  # "Yes" to next level
                            reset_game(reset_start_goal=True)
                            finished = False
                            start_time = None
                            elapsed_time = 0
                        ...
                        draw_text(win, "You lost press R to reset", (WIDTH // 2 - 150, HEIGHT // 2), font_size=30)
